...
//退出操作
function logout($scope,$http,DSHttpService,$state,$cookies,$cookieStore,commonjs){
...
//主界面菜单包括用户信息
function menu($scope,$rootScope,$state,$timeout,commonjs,DSHttpService){
    //登录成功后会保存一个用户信息到rootscope里，然后判断在之后的页面刷新时就取不到这个值，然后会调用取menu菜单的接口
    $scope.user = $rootScope.userInfo;
    if($scope.user){
        $scope.$watch('user',function(){
            $scope.menuList = $scope.user.adminMenus;
        })
        var sideNav = angular.element('#side-menu');
        $timeout(function(){
            sideNav.metisMenu();
        });
    }else{
        var promise = DSHttpService.httpPost(DSCONST.APIBASEURL+'menu/getMyMenus',{});
        promise.then(function(data){
            $scope.user = data.data;
            $scope.menuList = data.data.adminMenus;
            var sideNav = angular.element('#side-menu');
            $timeout(function(){
                sideNav.metisMenu();
            });
